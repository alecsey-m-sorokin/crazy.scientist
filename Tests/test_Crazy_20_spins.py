...
def test_01_crazy_slots(betSum, cntLineBet):
    creditDebit = api.CreditDebit(regToken, betSum,
                                  cntLineBet)  # ставка ! CreditDebit # resultId = tokenAsync
    tokenAsync = creditDebit["TokenAsync"]
    time.sleep(2)
    getAsyncResponse = api.GetAsyncResponse(regToken,
    ...
    if getAsyncResponse["SpinResult"]["DiceGame"] is None:
        ...
        while ThrowsLeft > 0:
            ...
            if WinType == 7:
                ...
                selectCardBonusGame = api.SelectCardBonusGame(regToken, resultId, bonusGameId, spinId,
                                                              A.CardIndex,
